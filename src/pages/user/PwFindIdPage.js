...
import styles from "./PwFindPhonePage.module.css";
import apiClient from "../../utils/axios"; // axios 인스턴스 추가
import axios from "axios"; // axios 인스턴스 추가
...
const PwFindPhone = () => {
    ...
    if (formData.name && formData.phone) {
      ...
      try {
        ...
        const response = await axios.post(
          "http://localhost:8888/seems/api/user/verification",
          {
        ...
        if (response.data.success) {
          ...
          console.log(
            isResend ? "인증번호 재전송 완료" : "인증번호 최초 전송 완료"
          );
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        // 인증 성공 후 바로 비밀번호 재설정 시도
        console.log("인증 성공 후 바로 비밀번호 재설정 시도");
  ...
  const handleResetPassword = async () => {
    if (
      ...
      // setResultMessage('인증 완료 후 이름, 아이디, 새 비밀번호를 입력해주세요.');
      return;
    }
    ...
    try {
      // 토큰이 필요없는 공개 API이므로 직접 axios 사용
      const response = await axios.post(
        "http://localhost:8888/seems/api/user/verification",
        {
          verificationType: "FIND_PASSWORD",
      ...
      if (response.data.success) {
        setResultMessage(
          response.data.message || "비밀번호가 재설정되었습니다."
        );
        ...
        setResultMessage(response.data.message || "비밀번호 재설정 실패");
      }
    } catch (err) {
      console.error("비밀번호 재설정 에러:", err);
      console.error("에러 응답:", err.response?.data);
      setResultMessage(err.response?.data?.message || "비밀번호 재설정 실패");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className={styles.forgotContainer}>
      ...
      <form className={styles.forgotForm} onSubmit={(e) => e.preventDefault()}>
        ...
        <div className={styles.formGroup}>
          ...
          <div className={styles.verifyGroup}>
            ...
            <button
              ...
              disabled={!isCodeSent || timer === 0 || isLoading}
            >
              확인
            </button>
          </div>
        ...
        {showSuccessMessage && (
          <div className={styles.successMessage} style={{ display: "none" }}>
            인증번호가 문자로 전송되었습니다.
            <br />
            문자를 확인해주세요.
          </div>
