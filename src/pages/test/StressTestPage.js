...
function StressTestPage() {
  ...
  const handleSubmitTest = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        "/seems/api/psychological-test/submit-stress-test",
        requestBody
      );
      ...
      alert("스트레스 검사가 완료되었습니다! 결과를 확인합니다.");

      // ✨ 2. URL 파라미터 수정
      navigate(
        `/psychological-test/result/${resultId}?type=${resultTestType || "STRESS_SCALE"}`
      );
    } catch (err) {
      console.error(
        "검사 제출 실패:",
      ...
      setError("검사 제출 중 오류가 발생했습니다. 잠시 후 다시 시도해주세요.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (questions.length === 0) {
    return (
      <div className={styles.container}>
        <UserHeader />
        <p>표시할 스트레스 검사 문항이 없습니다. 관리자에게 문의하세요.</p>
      </div>
    );
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.testCard}>
        <h1 className={styles.title}>스트레스 검사</h1>
        <p className={styles.description}>
          아래 문항을 읽고 지난 한 달간의 경험에 가장 가깝다고 생각하는 답변을
          선택해주세요.
        </p>
