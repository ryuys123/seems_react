...
const getLlamaResponse = async (messages, currentCoreQuestionIndex) => {
  ...
  try {
    ...
    return data; // response와 next_core_question_index를 모두 반환
  } catch (error) {
...
const CounselingPage = () => {
  ...
  useEffect(() => {
    const fetchInitialMessage = async () => {
      ...
      try {
        ...
        const data = await getLlamaResponse([], 0); 
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const data = await getLlamaResponse(messagesForAI, currentCoreQuestionIndex);
