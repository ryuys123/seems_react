  ...
  const checkUserIdDuplicate = async (userId) => {
    ...
    try {
      ...
      if (error.response?.status === 400) {
        alert(
          error.response.data.message || "아이디 중복 체크에 실패했습니다."
        );
      } else if (error.response?.status === 401) {
        // 401 에러는 백엔드에서 인증을 요구하는 경우
        // 아이디 중복 체크는 인증이 필요하지 않아야 함
        console.log(
          "백엔드에서 인증을 요구함 - 아이디 중복 체크는 인증 불필요"
        );
        alert(
          "아이디 중복 체크 중 인증 오류가 발생했습니다. 다시 시도해주세요."
        );
  ...
  const handleLinkAccount = async () => {
    try {
      ...
      if (response.data.success !== undefined) {
        ...
        response.data.message &&
        response.data.message.includes("연동")
      ) {
        isSuccess = true;
          ...
          {linkMode === "select" && (
            <div className={styles.selectSection}>
              <div className={styles.socialInfo}>
                <h3>소셜 계정 정보</h3>
                <p>
                  <strong>소셜 계정:</strong> {socialData?.provider}
                </p>
                <p>
              ...
              <div className={styles.choiceSection}>
                <h3>연동 방법 선택</h3>
                <p>
                  기존 계정에 연동하시겠습니까, 아니면 새 계정을 만드시겠습니까?
                </p>
            ...
            <div className={styles.confirmSection}>
              <div className={styles.accountInfo}>
                ...
                <div className={styles.accountDetails}>
                  <p>
                    <strong>아이디:</strong> {existingAccount.userId}
                  </p>
                  <p>
                    <strong>이름:</strong> {existingAccount.userName}
                  </p>
                  <p>
                    <strong>이메일:</strong> {existingAccount.email}
                  </p>
                </div>
              </div>
              ...
              <div className={styles.confirmButtons}>
                ...
                <button className={styles.cancelButton} onClick={handleGoBack}>
                  이전
                </button>
              </div>
            </div>
            ...
            <div className={styles.createSection}>
              ...
              <form onSubmit={handleSignup} className={styles.signupForm}>
                ...
                <div className={styles.inputGroup}>
                  ...
                  {idCheckStatus.checked && idCheckStatus.available && (
                    <span className={styles.availableIndicator}>
                      ✓ 사용 가능한 아이디입니다
                    </span>
                  )}
                  {idCheckStatus.checked && !idCheckStatus.available && (
                    <span className={styles.unavailableIndicator}>
                      ✗ 이미 사용 중인 아이디입니다
                    </span>
                  )}
                  {!idCheckStatus.loading &&
                ...
                <div className={styles.inputGroup}>
                  ...
                  <input
                    ...
                    required
                  />
                  {signupForm.userPwdConfirm &&
                    signupForm.userPwd !== signupForm.userPwdConfirm && (
                      <span className={styles.unavailableIndicator}>
                        비밀번호가 일치하지 않습니다
                      </span>
                    )}
